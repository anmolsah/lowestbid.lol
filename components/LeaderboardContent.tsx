...
export default function LeaderboardContent({ initialData }: LeaderboardContentProps) {
  ...
  // Leaderboard data — initialized from server-fetched props (no loading state needed)
  const [reigningChampion, setReigningChampion] = useState<BidItem | null>(initialData.reigningChampion);
  const [uniqueBids, setUniqueBids] = useState<BidItem[]>(initialData.uniqueBids || []);
  const [clashedBids, setClashedBids] = useState<BidItem[]>(initialData.clashedBids || []);
  const [highRollers, setHighRollers] = useState<BidItem[]>(initialData.highRollers || []);
  const [recentFeed, setRecentFeed] = useState<BidItem[]>(initialData.recentFeed || []);
  const [stats, setStats] = useState<LeaderboardStats>(initialData.stats || {
    totalVolume: 0,
    totalBids: 0,
  ...
  // Background polling for live updates (data is already available from server)
  ...
  useEffect(() => {
    // Start polling for live updates (no initial fetch needed — we have server data)
    ...
    <>
      ...
      <footer className="site-footer">
        ...
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', justifyContent: 'center', marginTop: '8px', fontSize: '0.85rem' }}>
          ...
          <span>&bull;</span>
          <span>Total Visitors: <strong>{(stats.totalVisitors || 0).toLocaleString()}</strong></span>
          <span>&bull;</span>
